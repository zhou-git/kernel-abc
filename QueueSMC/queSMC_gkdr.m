...
index=0;                           %sepreate summary statistics
%%%%%%%%%%%%%%%%%%%%% parameters for gkdr
dim_prop=5;

fix=0;
focuspara=0;
para_gkdr=[0.6 0.3 0.0001];
prop_step=[0.5 0.5 1.5];
%%
%ABC main process
...
for i=1:round
    ...
    [B,B_f,para_gkdr] = GKDR(dim_prop,fix,obstemp,focuspara,para_gkdr);%Pxnp 12x3 in this case
    %% here begin the smc
 ...
 % final target
   ...
   % npa_old=zeros(1,N);
    ...
    x_dis=zeros(M,N);
    for i2=1:M
        x_dis(i2,:)=rssq(x(:,:,i2)*B-SS_m(:,:,i2)*B,2);  %distance between x and obsSS, root of squared sum
    end
    w=ones(1,N)./N;
    npa_old=M.*ones(1,N); 
...
%	        break;
        ...
        if (sum(w.^2)*esstresh>1)
            ...
            fprintf('\n resample %d, Accrate: %f, Unique particles %f',counter, accrate_store,num_pa_tmp);
            fprintf('\n rssei %f %f %f\n',Rssei_tmp);
            if (mod(counter,5)==0 && plot_switch==1 &&i==1)
                for k = 1:3 
                    figure(k); 
                    histfit(theta(k,:),20,'kernel'); 
                    temp=['gkdrtheta ',num2str(k),' at resample: ',num2str(counter),'.eps'];                    
                    temp2=['gkdrtheta ',num2str(k),' at resample: ',num2str(counter)];      
                    xlabel(temp2);
                    saveas(gcf,temp);
                end 
            end
        ...
        SS_m=repmat(obstemp,[N 1 M]);
        for i2=1:M
            x_dis(i2,:)=rssq(x_prop(:,:,i2)*B-SS_m(:,:,i2)*B,2);  %distance between x and obsSS, root of squared sum
        end
        %w=ones(1,N)./N;
        %npa_old=M.*ones(1,N); 
        %d=abs(x_prop);
    ...
    if(i==1&&plot_switch==1)
        for k = 1:3 
            figure(k); 
            histfit(theta(k,:),20,'kernel'); 
            temp=['gkdr theta ',num2str(k),'.eps'];            
            temp2=['gkdr theta ',num2str(k)];      
            xlabel(temp2);
            saveas(gcf,temp);
        end
        ...
        saveas(gcf,'gkdrepsilon.eps');
        figure(5)
        plot(essstore(1,2:end));
        saveas(gcf,'gkdrESS.eps');
        figure(6)
        plot(accstore(1,2:end));
        saveas(gcf,'gkdrAcceptance rate.eps');
        figure(7)
        plot(num_par_store(1,2:end));
        saveas(gcf,'gkdrAcceptance rate.eps');
    end
    toc2=cputime-tic2;
    fprintf('\nRound: %d, iteration: %d\n resample: %d, cpu time:  %d \n epsilon: %f,  Unique partical: %d\n',i, counter2, counter,toc2,epsilon,num_pa_tmp);
   % Rssei(:,i) =sum(minus(theta,obstheta).^2,2);
