...
epsilontarget=0.01; % final target
...
for i=1:round
    ...
    B = pilotrej(obstemp);%Pxnp 12x3 in this case
   ...
   % npa_old=zeros(1,N);
	...
	if(num_pa_tmp<=300)
        ...
        if (sum(w.^2)*esstresh>1)
            ...
            fprintf('\n resample %d, Accrate: %f, epsilon:%f, Unique particles %f',counter, accrate_store,epsilon,num_pa_tmp);
            fprintf('\n rssei %f %f %f\n',Rssei_tmp);
    ...
    if(i==1&&plot_switch==1)
        for k = 1:3 
            figure(k); 
            histfit(theta(k,:),20,'kernel'); 
            temp=['semitheta ',num2str(k),'.eps'];            
            temp2=['semitheta ',num2str(k)];      
            xlabel(temp2);
            saveas(gcf,temp);
        end
        ...
        saveas(gcf,'semiepsilon.eps');
        figure(K+2)
        plot(essstore(1,2:end));
        saveas(gcf,'semiESS.eps');
        figure(k+3)
        plot(accstore(1,2:end));
        saveas(gcf,'semiAcceptance rate.eps');
        figure(k+4)
