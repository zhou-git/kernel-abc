...
.compute_dist <- function(summary_stat_target, simul, sd_simul, dist_weights=NULL) {
    ...
    # are set to one
    vartab[vartab == 0] = 1
    colSums((t(simul) - as.vector(summary_stat_target))^2/as.vector(vartab),na.rm = TRUE)
}
...
.compute_dist_M <- function(M, summary_stat_target, simul, sd_simul, dist_weights=NULL) {
    ...
    if (l > 1) {
      ...
      #      vartab[i] = min(1, 1/(sd_simul[i] * sd_simul[i]))  ## differences between simul and data are normalized in each dimension by the empirical variances in each dimension
            dist = dist + (simul[, i] - summary_stat_target[i]) * (simul[, 
                i] - summary_stat_target[i])  ## an euclidean distance is used#vartab[i]
        }
    } else {
     #   vartab = min(1, 1/(sd_simul * sd_simul))  ## differences between simul and data are normalized in each dimension by the empirical variances in each dimension
        dist = dist +  (simul[, 1] - summary_stat_target) * (simul[, 1] - summary_stat_target)  ## an euclidean distance is used
    }
...
.compute_weight_delmoral <- function(particle_dist_mat, tolerance) {
    ...
    for (i in 1:n_particle) {
        t=particle_dist_mat[i, ][particle_dist_mat[i, ] < tolerance]
        new_weight[i] = length(t)-sum(is.na(t))
    }
...
.ABC_Delmoral <- function(model, prior,  nb_simul, summary_stat_target, tolerance_target = 1,g_flag,pB,
    verbose=TRUE,use_seed,n_cluster,prior_test, dist_weights=NULL,alpha = 0.9, M = 5, nb_threshold = floor(nb_simul/5),
     seed_count = 0, progress_bar = FALSE, max_pick=10000) {
    ## checking errors in the inputs
    if (!is.vector(alpha)) 
        stop("'alpha' has to be a number.")
    if (length(alpha) > 1) 
    ...
    # step 1 classic ABC step
     simul_below_tol = .ABC_rejection_M(model, prior, prior_test, nb_simul, M, use_seed, 
         seed_count)
     save(simul_below_tol,file="simu.Rdata")
    #load("simu.Rdata")

    simul_below_tol[!is.finite(simul_below_tol)]=NA
    ...
    #sd_simul1=apply(simul_below_tol[,-(1:3)],2,sd,na.rm=TRUE)
    #mean_simul=apply(simul_below_tol[,-(1:3)],2,mean,na.rm=TRUE)
    load("rickerFIX.Rdata")
    if(ss_set==1){
        theta_obs=c(obs.log_r[1],obs.log_sig_e[1],obs.phi[1])
        sd_simul=sd_E1
        mean_simul=mean_E1
    }
    if(ss_set==0){
    ...
    sd_simul[sd_simul==0]=1
    summary_stat_target=(summary_stat_target-mean_simul)/sd_simul
    simul_below_tol[,-(1:3)]=sweep(simul_below_tol[,-(1:3)],2,mean_simul,FUN="-")
    simul_below_tol[,-(1:3)]=sweep(simul_below_tol[,-(1:3)],2,sd_simul,FUN="/")
    if(g_flag>0){
      simul_below_tol=cbind(simul_below_tol[,1:nparam],simul_below_tol[,-(1:nparam)]%*%pB)
      summary_stat_target=summary_stat_target%*%pB
      nstat=length(summary_stat_target)
    }
    l = dim(simul_below_tol)[2]
    ...
    new_tolerance = max(particle_dist_mat,na.rm = TRUE)
    ...
    # }
    #if (progress_bar) {
    end1.time =sum(proc.time()-start.time)
    print(paste("step 1 completed,  ", "time = ", end1.time))
    #}
    # following steps
    kstep = 1
    verbose_count=1
    while (new_tolerance > tolerance_target) {
        ...
        for (i in 1:nb_simul) {
            if (tab_weight[i] > 0) {
                ...
                for (j in 1:M) {
                  simu_t=model(param)
                  simu_t=(simu_t-mean_simul)/sd_simul
                  if(g_flag>0){
                     simu_t=simu_t%*%pB
                  }
                  new_simul = c(param, simu_t)
                  new_simul[!is.finite(new_simul)]=NA
                  if (use_seed) {
                    param[1] = param[1] + 1
                  }
                  seed_count = seed_count + 1
        ...
        if (verbose == TRUE && verbose_count%%10==1) {
            write.table(as.numeric(new_tolerance), file = paste("./output/tolerance_step", 
                kstep, sep = ""), row.names = F, col.names = F, quote = F)
            write.table(as.matrix(cbind(tab_weight2, simul_below_tol)), file = paste("./output/output_step", 
                kstep, sep = ""), row.names = F, col.names = F, quote = F)
            write.table(as.numeric(seed_count - seed_count_ini), file = paste("./output/n_simul_tot_step", 
                kstep, sep = ""), row.names = F, col.names = F, quote = F)
       ...
       # if (progress_bar) {
            data_matrix_sse=cbind(tab_weight2, simul_below_tol)
            sse=((t(data_matrix_sse[,1])%*%(data_matrix_sse[,2:(nparam+1)]))/M-theta_obs)^2
            current.time=sum(proc.time()-start.time)
            print(paste("step ", kstep, " completed - tol =", new_tolerance,"   process time= ",current.time,"   sse1 = ",sse[1], "   sse2 = ",sse[2],"   sse3 = ",sse[3],sep = ""))
            if(new_tolerance<tolerance_target){
              cat("finish!")
            }
       # }
    }
...
## 2015)
.ABC_sequential <- function(method, model, prior, nb_simul, summary_stat_target, tolerance_target,
    g_flag,pB,verbose=NULL,use_seed,n_cluster,prior_test=NULL,dist_weights=NULL, ...) {
    options(scipen = 50)
    ...
    return(switch(EXPR = method, Beaumont = .ABC_PMC(model, prior, prior_test, nb_simul, 
        ...
        Delmoral = .ABC_Delmoral(model, prior, nb_simul, summary_stat_target, tolerance_target,
                                 g_flag, pB,
             verbose,use_seed,n_cluster,prior_test,dist_weights=dist_weights,
            ...), Lenormand = .ABC_Lenormand(model, prior, prior_test, 
...
.ABC_rejection_M_cluster <- function(model, prior, prior_test, nb_simul, M, seed_count, 
    ...
    cl <- makeCluster(getOption("cl.cores", n_cluster),outfile='parellallog.txt')
    l = length(prior)
    param2 = array(0, (l + 1))
    if (npar > 0) {
        for (irun in 1:npar) {
...
## sequential algorithm of Del Moral et al. 2012 - the proposal used is a normal
## in each dimension (cf paragraph 3.2 in Del Moral et al. 2012)
.ABC_Delmoral_cluster <- function(method, model, prior, nb_simul, summary_stat_target, tolerance_target,g_flag, pB,
  verbose,use_seed,n_cluster,prior_test, dist_weights=dist_weights, alpha = 0.9, M = 5, nb_threshold = floor(nb_simul/5),
  seed_count = 0, progress_bar = FALSE, max_pick=10000) {
    ## checking errors in the inputs
    if (!is.vector(alpha)) 
        stop("'alpha' has to be a number.")
    ...
    verbose_count=1
    l = length(prior)
    cl <- makeCluster(getOption("cl.cores", n_cluster),outfile="logparallal.txt")
    while (new_tolerance > tolerance_target) {
        kstep = kstep + 1
        # determination of the new tolerance
        ESS_target = alpha * ESS
        ...
        tab_new_simul=cbind(tab_new_simul[,1:nparam], tab_new_simul[,-(1:nparam)]/(sd_simul*sd_simul))
        if(g_flag>0){
          tab_new_simul=cbind(tab_new_simul[,1:nparam], tab_new_simul[,-(1:nparam)]%*%pB)
        }
        tab_new_simul[!is.finite(tab_new_simul)]=NA
        ...
        if (verbose == TRUE && (verbose_count&&10==1)) {
            write.table(as.numeric(new_tolerance), file = paste("./output/tolerance_step", 
                kstep, sep = ""), row.names = F, col.names = F, quote = F)
            write.table(cbind(tab_weight2, simul_below_tol), file = paste("./output/output_step", 
                kstep, sep = ""), row.names = F, col.names = F, quote = F)
            write.table(as.numeric(seed_count - seed_count_ini), file = paste("./output/n_simul_tot_step", 
                kstep, sep = ""), row.names = F, col.names = F, quote = F)
            intermediary_steps[[kstep]] = list(n_simul_tot = as.numeric(seed_count - 
...
.ABC_sequential_cluster <- function(method, model, prior, nb_simul, summary_stat_target, tolerance_target,
                                    g_flag, pB,
                                    verbose,use_seed,n_cluster,prior_test, dist_weights=NULL, ...) {
    if (use_seed == FALSE) {
    ...
    return(switch(EXPR = method, Beaumont = .ABC_PMC_cluster(model, prior, prior_test, 
        ...
        prior, nb_simul, summary_stat_target, n_cluster, verbose, dist_weights=dist_weights, ...), 
        Delmoral = .ABC_Delmoral_cluster(method, model, prior, nb_simul, summary_stat_target, tolerance_target,
                                         g_flag, pB,
                                         verbose,use_seed,n_cluster,prior_test, dist_weights=dist_weights, ...), 
