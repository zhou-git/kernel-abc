...
void ng_bf_old(double *n,double *theta,double *e,int *burn_in,int *n_t, int *n_reps) {
/* Simulates `n_reps', length `n_t' replicates of blowfly model, discarding an initial 
   sequence of length `burn_in'.
   ...
   Note that error here is in birth not death. Death case would require care to
   avoid unbiological spontaneous generation.
*/
  int i,j,lag;
  double delta,P,N_0,sig,tau,*N,*N1,*N0,*Nlag,err=0.0,one_alpha,alpha;
  delta = theta[0];
  P = theta[1];
  N_0 = theta[2];
  sig = theta[3];
  tau = theta[4]; /* note that this is discrete */
  alpha = theta[5]; /* the autoregression parameter */
  one_alpha = 1 - alpha;
  lag = (int) floor(tau);if (tau-lag>.5) lag++;
  /* Allocate single replicate storage */
  N = (double *)calloc((size_t)lag + *burn_in + *n_t,sizeof(double));
  ...
  for (j=0;j<*n_reps;j++) {
    N1 = N + lag;N0 = N1 - 1;Nlag=N;
    err = *e;
    for (i=lag;i< *burn_in+lag;i++,N0++,N1++,Nlag++) {
      /* This is old discretization, less stable, and less natural than one used now...
         *N1 = *N0 * exp(P * *Nlag * exp(- *Nlag/ N_0)/ *N0 - delta + err * sig); */
      
      *N1 = (P * *Nlag * exp(- *Nlag/ N_0) + *N0 * exp( - delta ))*exp(err * sig);
      err *= alpha;e++;err += *e * one_alpha;
    }
    for (i=0;i<*n_t;i++,n++,N0++,N1++,Nlag++) {
      /* *n =  *N1 = *N0 * exp(P* *Nlag*exp(- *Nlag/N_0)/ *N0 - delta + err * sig); */
      *n =  *N1 =  (P * *Nlag * exp(- *Nlag/ N_0) + *N0 * exp( - delta ))*exp(err * sig);
      err *= alpha;e++;err += *e * one_alpha;
    }
  }
  free(N);
